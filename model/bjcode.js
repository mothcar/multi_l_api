...
const initSchema = async ()=>{
  ...
  const bjcode = new mongoose.Schema({
  ...
  try{
    ...
    let index = _.findIndex(list, {name:'bjcode'})
    if(index < 0)
      bjcode.index({
        email:1,
        enabled:1,
      ...
      log('init schema (bjcode): collection found. creation skipped')

    global.Bjcode = mongoose.model('bjcode', bjcode)
